# Integrating GridLadder

## From Solidity

```solidity
import {GridLadderHook} from "grid-ladder/src/hooks/GridLadderHook.sol";
...
const hook = getHook("grid-ladder");
const address = hookAddress("grid-ladder", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "GridLadder"
...
More at https://grid-ladder.pages.dev.
